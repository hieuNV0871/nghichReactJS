...
const FooterSidebar = (props) => {
  return (
    <div className="relative py-4 before:content-[''] before:right-2 before:left-2 before:h-[1px] before:absolute before:bg-[#1618231f] before:top-0">
      ...
      <span className="mt-[5px] px-2 text-xs text-[#16182380] font-semibold">
        © 2023 TopTop
      </span>
    </div>
  );
    ...
    <div className="px-2 mb-2">
      {props.data.map((d, index) => (
        <Button
          key={index}
          to={d.to}
          className="hover:underline mt-[5px] mr-[6px] text-xs text-[#16182380] font-semibold inline-block"
        >
          {d.title}
        </Button>
      ))}
