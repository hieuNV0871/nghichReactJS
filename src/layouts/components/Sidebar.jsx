...
const Sidebar = props => {
    ...
    useEffect(()=>{
      const fetchApi = async () => {
        const result = await userServices.SuggestedAccount({page: INIT_PAGE, perPage: perPage});
    ...
    return (
        <aside className='pl-4 pt-5 w-[356px] fixed z-10 bottom-0 top-16 bg-white  overflow-y-hidden hover:overflow-y-scroll'>
            ...
            <SuggestedAccounts title="Suggested accounts" label ={suggestedAccounts.length === PER_PAGE ? "See all" : "See less"} data={suggestedAccounts} onSeeAll = {handleSeeAll}/>
